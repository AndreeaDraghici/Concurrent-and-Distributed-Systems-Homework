...
public class IndustrySemaphores extends Thread {

    private int number;//numarul fabricii
    private int size;//dimensiunea fabricii
    private boolean industry[][];
    private final ArrayList<ElfPozitionSemaphores> elfArrayList = new ArrayList<>();//lista in care tinem pozitia elfilor
    ...
    public ReentrantLock getIndustryLock() {
        return industryLock;
    }


    public IndustrySemaphores(int number, int size) {
        this.number = number;
        this.size = size;
    ...
    private void reportThePosition() {
        try {
            ...
            */
            ...
            elfListLock.lock();
            giftLock.lock();
            Iterator<ElfPozitionSemaphores> iterator = elfArrayList.iterator();
            while (iterator.hasNext()) {
                ElfPozitionSemaphores elf = iterator.next();
                elf.reportElfPosition();
            }
        } catch (Exception e) {
            System.out.println(MessageFormat.format(e.getMessage(),
                    "Exceptie din industrie: {0}"));
            e.printStackTrace();
     ...
     */
    public boolean addElves(ElfPozitionSemaphores elfPozition) {
        var result = false;
        elfListLock.lock();
     ...
     */
    public void retireElf(ElfPozitionSemaphores elfPozition) {
